...
This AI-powered trip planner allows users to input their travel preferences like destination, dates, budget, and interests, and generates a customized itinerary. Users can refine the itinerary based on their feedback, leveraging OpenAI's GPT model and external APIs like Google Maps and OpenWeather.

In the below repo, we express 2 ways to executing this travel planner: via a Jupyter notebook, or via a Streamlit app. The Jupyter Notebook can also be executed via Kaggle, in which case the API keys are not needed.
